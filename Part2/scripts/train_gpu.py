...
# --- Device selection (the key GPU-enabling change vs. train.py) -----------
...
def quick_eval(model, loader_iter_fn, n_batches):
    model.eval()
    ious, dices = [], []
    with torch.no_grad():
        for xb, yb, _ in loader_iter_fn(n_batches):
            ...
            for p, t in zip(pred, yb):
                p_np = p[0].cpu().numpy().astype(np.uint8)
                t_np = t[0].cpu().numpy().astype(np.uint8)
...
def main():
    log_path = os.path.join(os.path.dirname(__file__), "..", "train_log_gpu.txt")
    logf = open(log_path, "w")

    log_line(logf, f"Device: {device}" + (f"  ({torch.cuda.get_device_name(0)})" if device.type == "cuda" else ""))
    log_line(logf, f"CPU count: {os.cpu_count()}  |  CUDA available: {torch.cuda.is_available()}")
    ...
    val_loader_full = DataLoader(val_ds, batch_size=BATCH, shuffle=True, num_workers=0, pin_memory=pin)
    ...
    with open(os.path.join(os.path.dirname(__file__), "..", "train_history_gpu.csv"), "w", newline="") as f:
    ...
    meta = {
        ...
        "train_patches": len(train_ds), "val_patches": len(val_ds), "device": str(device),
    }
    with open(os.path.join(os.path.dirname(__file__), "..", "train_meta_gpu.json"), "w") as f:
        json.dump(meta, f, indent=2)

    log_line(logf, f"\nTRAINING DONE on {device}. best quick-val IoU={best_iou:.4f}")
    logf.close()
